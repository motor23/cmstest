...
class Query:
    ...
    def clone(self, keys=None, query=None, items=None):
        ...
        if items is None:
            items = self._items
        return self.__class__(self._mapper,
            keys=keys,
            query=query,
            items=items,
        )
    ...
    def _execute_query(self, db): raise NotImplementedError
    def _execute_items(self, db): raise NotImplementedError
...
class SelectQuery(Query):
    ...
    def _execute_query(self, db):
        result = list(map(lambda row: dict(row), db.execute(self._query)))
        row_by_id = dict(map(lambda row: (row['id'], row), result))
    ...
    def _execute_items(self, db):
        assert not list(filter(lambda x: 'id' not in x, self._items))
        ids = map(lambda x: x['id'], self._items)
        query = self._query.where(self._mapper.table.c.id.in_(ids))
        result = list(map(lambda row: dict(row), db.execute(query)))
        row_by_id = dict(map(lambda row: (row['id'], row), result))
...
class Mapper:
    ...
    def div_keys(self, keys=None):
        ...
        table_keys = set(filter(lambda x: x in self.table.c, keys))
        table_keys.add('id')
        relation_keys = set(filter(lambda x: x in self.relations, keys))
    ...
    def store(self, db, items, keys=None):
        ...
        ids = map(lambda x: x.get('id'), items)
        ids = set(filter(lambda x: x is not None, ids))
        result = self.select(['id']).filter_by_id(*ids).execute(db)
        update_ids = set(map(lambda x: x['id'], result))
        ins_items = list(filter(lambda x: x.get('id') not in update_ids, items))
        up_items = list(filter(lambda x: x.get('id') in update_ids, items))
